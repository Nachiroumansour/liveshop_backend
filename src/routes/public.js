...
// Route pour récupérer les informations d'un vendeur public
router.get('/sellers/:linkId', validatePublicLink, async (req, res) => {
  try {
    const { linkId } = req.params;
    ...
    console.error('Erreur lors de la récupération du vendeur public:', error);
    res.status(500).json({
      error: 'Erreur interne du serveur'
    });
  }
...
// Route pour récupérer les informations d'un live public
router.get('/lives/:liveId', async (req, res) => {
  try {
    const { liveId } = req.params;

    const live = await require('../models').Live.findByPk(liveId, {
      include: [
        {
          model: require('../models').Seller,
          attributes: ['name', 'public_link_id']
        }
      ]
    });
    ...
    console.error('Erreur lors de la récupération du live public:', error);
    res.status(500).json({
      error: 'Erreur interne du serveur'
    });
  }
...
// Route pour récupérer les produits d'un live public
router.get('/lives/:liveId/products', async (req, res) => {
  try {
    const { liveId } = req.params;
    ...
    const live = await require('../models').Live.findByPk(liveId, {
      include: [
        {
          model: Product,
          through: { model: require('../models').LiveProduct }
        }
      ]
    });
    ...
    console.error('Erreur lors de la récupération des produits du live public:', error);
    res.status(500).json({
      error: 'Erreur interne du serveur'
    });
  }
